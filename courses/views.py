...
def chapter(request, course_name=None, slug=None):
    ...
    queryset_quiz = Quiz.objects.filter(chapter__chapter_name=place.chapter_name)
...
@user_passes_test(lambda user: user.is_professor)
def quiz(request, quiz_id=None, question_id=None, answer=None):
    quiz = Quiz.objects.get(id=quiz_id)
    add_question_form = AddQuestionForm(request.POST or None)
    
    if answer!='None':
        obj = Answers.objects.create()
...
@user_passes_test(lambda user: user.is_professor)
def generate_quiz(request, txt_block_id=None,slug=None):
    instance = TextBlock.objects.get(id=txt_block_id)
    chapter = Chapter.objects.get(slug=slug)
    questions = get_relevent_questions(instance.lesson)
    temp1 = Quiz.objects.create()
    temp1.chapter=chapter
    temp1.title='generated Quiz'
    temp1.save()
    for question in questions:
        temp2 = Question.objects.get(id=question)
        temp2.quiz=temp1
        temp2.save()
    return HttpResponseRedirect(request.META.get('HTTP_REFERER'))
